...
export default {
  ...
  methods: {
    ...
    async removeGridItem(el) {
      ...
      if (document.getElementById(el.id).parentNode.id == el.grid_id)
        this.grid.removeWidget(document.getElementById(el.id));
      else {
        console.log(document.getElementById(el.id).parentNode.id);
        // console.log(el.grid_id)
        //el.grid_id = document.getElementById(el.id).parentNode.id
    ...
    //   );
    // },
    changeItemGrid(event, oldWidget, newWidget) {
      console.log("CHANGE DISPLAY");
      let oldItem = { id: oldWidget.el.id, grid_id: oldWidget.grid.el.id };
      let newItem = {
        id: newWidget.el.id,
        grid_id: newWidget.grid.el.id,
        ["x_axis"]: newWidget.x,
        ["y_axis"]: newWidget.y,
        height: newWidget.h,
        width: newWidget.h,
